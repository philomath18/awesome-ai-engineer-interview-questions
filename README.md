...
A base model is trained on exactly one objective, predicting the next token, so it continues the pattern of the text rather than responding to it. In its training data a question is very often followed by more questions, in quiz lists and FAQs, so the most likely continuation of "What is the capital of France?" is another trivia question rather than "Paris."  

**Q: What's the difference between training and inference?**  `Foundations`  
Training is the learning phase. The model reads data, computes a loss on its next-token guesses, and uses backpropagation to update billions of weights. Inference is the serving phase. The weights are frozen and the model runs a single forward pass to answer a prompt, cheap per call but happening on every request.  

**Q: What's the difference between an embedding and a generative model?**  `Foundations`  
An embedding model maps a piece of text to a single fixed-length vector that captures its meaning. It returns numbers rather than words, and you use it to measure similarity for search, clustering, and retrieval. A generative model produces text one token at a time, and you use it to write, answer, summarize, or translate.  

**Q: Walk me through how a model like ChatGPT is built.**  `Foundations`  
It is built in three training stages, each editing the weights. Pretraining reads a huge slice of the internet predicting the next token, and picks up language and world knowledge as a side effect. The output is a base model. Instruction tuning (SFT) shows it curated pairs of an instruction and an answer, so it learns to respond helpfully. Alignment (RLHF or DPO) tunes tone, safety, and judgement from human preferences.  
...
The context window is the maximum number of tokens a model can attend to in a single call. It is the model's working memory, and it is shared across the system prompt, the conversation history, any pasted documents, and the reply the model generates. When the total exceeds the window, the oldest tokens fall off and are gone entirely.  

**Q: What are logits, and how do they differ from the probabilities the model ultimately uses?**  `Foundations`  
A logit is the model's raw confidence score for one candidate token, and the final layer emits one per token in the whole vocabulary. Logits are unbounded, they can be negative or large, and they do not sum to anything, so they are really just a ranking. Probabilities are what you get after softmax: values between 0 and 1 that add up to exactly 1, which you can actually sample from.  
...
A model only does math over numbers, so raw letters mean nothing to it. Tokenization bridges that. It splits your text into small pieces, usually subwords, drawn from a fixed vocabulary, then swaps each piece for its integer ID.  
...
Self-attention only computes how much each word should attend to each other word, and that quantity doesn't depend on the order of the inputs. It treats them like a bag of words. Permute the tokens and the attention scores come back the same, which is why we call it order-blind, or permutation-invariant. The concrete cost: without help the model literally cannot distinguish "dog bites man" from "man bites dog".  

**Q: Describe the autoregressive generation loop step by step, and when does it stop?**  `Foundations`  
You start with a prompt. One forward pass produces logits, a score for every possible next token. You turn those into probabilities and pick one. You append that token to the end and run the whole longer sequence through the model again. It stops when the model emits a special stop token, also called the end-of-sequence token, or when it reaches a length limit.  

**Q: What does the temperature dial control, and what happens at 0 versus above 1?**  `Foundations`  
Temperature stretches or squashes the odds before sampling: you divide the logits by it. Below 1, the gaps between options grow, the top word dominates, and text stays focused and safe. Above 1, the gaps shrink, unlikely words get a real shot, and text turns surprising and riskier. At exactly 0 it collapses to greedy decoding, always the single most likely token, which is fully deterministic: same prompt, same output.  
...
There are three families. Encoder-only models like BERT, RoBERTa, and DeBERTa read bidirectionally and are built for understanding: classification, tagging, embeddings. Decoder-only models like GPT, Llama, and Claude generate left to right and are built for writing, and this is the shape behind every modern chat LLM. Encoder-decoder models like T5 and BART pair an encoder that reads the input with a decoder that writes the output, built for text-to-text jobs like translation and summarization.  
...
A residual connection computes x + f(x) instead of just f(x), so the original signal is always carried forward. Going forward it reaches deep layers without fading, and going backward the gradient gets a clean path home, which sidesteps the vanishing gradient problem. Normalization rescales each vector so its values sit in a predictable range, stopping activations from exploding or collapsing as they flow through the stack.  
...
After the final layer the model hands you logits, one raw score per word in the vocabulary, and softmax turns them into probabilities. That distribution isn't an answer yet. You still have to choose one token, and decoding is exactly that policy: always take the top score with greedy, keep several candidate sentences with beam, or draw at random weighted by the odds with sampling.  
...
A block has exactly two sublayers. Multi-head self-attention lets each token look at the others and pull in context, and it is the only place information moves between positions. The feed-forward network (FFN) then takes each token on its own and runs it through a small two-layer network, the per-token thinking step.  

**Q: What does running multiple attention heads let the model do that a single reader couldn't?**  `Applied`  
A single attention pass can only emphasise one pattern at a time. Multi-head attention slices the vectors into several heads, each with its own Query/Key/Value, and they run in parallel. One head can specialise in grammar, another in resolving which pronoun points to which noun, another in long-range topic links.  

**Q: What does FlashAttention compute compared to standard attention, and what does it avoid writing to slow memory?**  `Advanced`  
It computes the exact same attention output. It is not an approximation. The difference is purely in data movement. Standard attention builds the full N×N score grid and writes it out to slow memory (HBM), then reads it back for the softmax and again to combine the values. FlashAttention never writes that grid out at all.  

**Q: What core problem with transformer attention do SSMs and linear attention set out to fix?**  `Advanced`  
Transformer attention compares every token to every other token, so compute grows with the square of the sequence length, and the KV cache of stored keys and values grows linearly with length on top of that. On short text you never notice. On a 200-page document, a long codebase, or a genome, both the compute and the memory explode. SSMs and linear attention break that quadratic curve by carrying a fixed-size running state instead, so cost grows linearly and memory stays flat.  
...
Zero-shot gives the model only the instruction and trusts it to comply. Few-shot pastes a handful of worked examples first, so the model copies the pattern. Few-shot is the simplest dependable boost whenever the exact format, style, or labelling matters.  
...
A model's knowledge is frozen at its training cutoff and never included your private data. So it can't answer about recent events or your own docs, and when pushed it tends to hallucinate a confident guess. RAG (retrieval-augmented generation) retrieves the most relevant real passages at question time and pastes them into the prompt, so the model answers from the page in front of it instead of from memory. You ground it on real text and instruct it to answer only from that text or say "I don't know", which sharply cuts made-up answers and lets it cite sources.  

**Q: What is chunking in a retrieval system, and why does bad chunking quietly wreck search?**  `Applied`  
Chunking is cutting your documents into small pieces, embedding each one, and storing them so search can compare a question to one piece at a time. It quietly decides quality because retrieval matches whole chunks. Cut too large and the relevant sentence is buried in filler, so the vector averages out and stops matching cleanly. Cut too small and one answer is split across pieces, so no single chunk holds it.  
...
There are three tools for three different jobs. RAG, retrieval-augmented generation, keeps facts outside the model and fetches the relevant ones at question time, so the model looks things up. Fine-tuning nudges the model's weights so it behaves a certain way, which bakes the behaviour in. Long context drops all the needed text into the prompt for one request, so you paste it in.  
...
It's a retrieve-then-rerank pipeline borrowed from classic search. Stage one casts a wide, cheap net tuned for recall. It just needs the right passage to land somewhere in the top ~50, and the order can be rough. Stage two is tuned for precision: a slower, sharper model re-reads that shortlist and re-sorts it so the best passage ends up first.  

**Q: Why does standard similarity-based RAG fail on 'how do X and Y relate across three documents?'**  `Applied`  
Similarity search returns the chunks that look most like the question, which is perfect for a plain "what is X" lookup. A relational, multi-hop question is different. Its answer is written in no single passage. It lives in the connections between facts scattered across several documents. So RAG hands back chunks about X and chunks about Y, and never the link between them, because that link was never in a single chunk to be matched.  

**Q: Why must you score retrieval and generation as two separate layers?**  `Applied`  
A RAG answer can fail in two independent places, and the final answer looks the same either way. Retrieval might fetch the wrong chunks. Or retrieval was fine but the generator misused good chunks. A single accuracy number blends the two, so you cannot tell which to fix, and you end up tuning chunking when the model was hallucinating, or swapping models when the search was broken.  
...
There are three stages, in order. Pretraining has the model read a massive pile of raw text predicting the next token, so it absorbs language, facts, and reasoning, though all it learns to do is continue text. SFT (supervised fine-tuning) has it imitate curated pairs of an instruction and an ideal answer, so it gains the behaviour of actually answering, in the right format. RLHF then tunes it on human rankings so it matches what people prefer in tone, helpfulness, and safety.  
...
LoRA freezes the entire base model and trains only a small pair of add-on matrices, B·A, on a few chosen layers. That is typically well under 1% of the weights. Because the base never changes, a finished task is just those tiny adapters, a few MB each. So you can keep dozens of task-specific versions sitting on one shared frozen base instead of storing a full copy of the model per task.  
...
Pretraining only teaches what word comes next, so a base model continues text. Ask it a question and it may write more questions. Instruction tuning fine-tunes it on many pairs of an instruction and a good answer, so it learns the general habit of doing what is asked.  
...
Three stages. First, collect preferences: humans compare pairs of answers to the same prompt and pick the better one. Second, train a reward model: a network learns from those picks to output a single score for how much a human would prefer a reply. Third, optimize with RL: the model, now called the policy, writes answers, the reward model scores them, and PPO nudges the weights toward higher scores.  

**Q: What makes a reward 'verifiable' in RLVR, and on what kinds of problems does that apply?**  `Advanced`  
A reward is verifiable when a program can decide whether the answer is correct, with no human opinion involved, yielding a clean 1 or 0. That applies wherever correct is well defined. In math you compare the final number to a known gold answer. In code you run it against unit tests.  

**Q: What is a 'constitution' in Constitutional AI, and how does the model use it?**  `Advanced`  
A constitution is a short, plain-language list of principles: be helpful, avoid harmful advice, be honest about uncertainty. Anthropic's draws some of its lines from sources like the UN Declaration of Human Rights. The model uses it to police itself. It drafts an answer, critiques that draft against a principle, and rewrites to fix the problem.  
...
Before MCP, every AI app needed its own bespoke connector to every tool. With N apps and M tools that is N times M one-off integrations to build and maintain, and a connector for one app was useless to the next. MCP (Model Context Protocol) defines one common protocol both sides speak. The N times M tangle collapses to a tidy N plus M, and anything new instantly works with everything that already exists.  
...
There are two layers. Orchestration frameworks run the agent's own steps and state. LangGraph is the most deployed and suits stateful, control-heavy loops, LlamaIndex fits agents that are mostly about your data, and plain LangChain covers quick chains. Interop protocols connect the agent outward: MCP to tools and data, A2A to other agents.  
...
Observability is descriptive: logs and traces of every step, tool call, and token. It tells you what happened and is great for debugging. Evaluation is the judgment layer on top, a repeatable way to score outcomes against what should have happened, whether that is pass/fail, a rubric, or a number. Adoption is lopsided, roughly 89% observability vs 52% evaluation, so most teams can watch their agents but never actually test them.  
...
There are three levels. Public benchmarks like MMLU, HumanEval, and GSM8K score the raw model against everyone, and they are good for picking a model. Your own task metrics, things like exact match, ROUGE, and pass@k on a test set built from real user tasks, prove it does your job, which no benchmark covers. Then LLM-as-judge grades the open-ended answers where no single string counts as correct.  

**Q: Why is there no single score for a good LLM output, and how do you choose a metric?**  `Applied`  
Language is open-ended. The same correct answer can be written a hundred ways, so no one number captures "good". Instead you match the metric to the task: word overlap for translation, execution tests for code, a rubric judge for chat.  

**Q: What specific capability does each of MMLU, GPQA, HumanEval, SWE-bench, and AIME actually measure?**  `Advanced`  
MMLU is a broad 57-subject multiple-choice quiz that measures general knowledge. GPQA asks hard graduate-level science questions that experts themselves find tricky. HumanEval, along with MBPP, checks whether the model can write a small function that passes hidden tests. SWE-bench is far more realistic: it hands the model an actual GitHub bug report and verifies its patch fixes the repo. AIME tests competition-math reasoning.  

**Q: Why is a bad LLM output called a symptom rather than a diagnosis, and what's the overall approach?**  `Applied`  
The same visible failure, a bad answer, can come from any layer of the pipeline: the prompt, the model, retrieval, the schema and parser, or a tool call. The screen shows you the end of the pipeline, never the origin. So the job is to turn one vague symptom into a specific, provable layer by reading the tell, whether that is malformed JSON, a 4xx, a made-up fact, or an ignored rule. Then you fix the cheapest layer first and confirm on a fixed eval before moving on.  
...
The KV cache stores the Key and Value vectors of every token the model has already seen. To generate the next token, attention has to look back at the K and V of all earlier tokens, and those never change once computed. Without a cache the model recomputes all of them from scratch every single step, which is pure repeated arithmetic.  

**Q: What does quantization change about how a model's numbers are stored, and what do you trade for it?**  `Applied`  
Quantization stores each weight using fewer bits, for example going from 16-bit fp16 down to 8-bit or 4-bit integers. It removes no weights and leaves the architecture alone. It just rounds each number to a coarser scale, keeping a scale factor to stretch it back at run time. The trade is a much smaller, faster model in exchange for a small rounding error that can chip at quality.  
...
Two things working together. PagedAttention stores the KV cache in small fixed-size pages handed out on demand, the way an operating system hands out virtual memory, so it barely wastes memory and fits many more requests. Continuous batching swaps a finished request out and a waiting one in at every single step, so the GPU never sits idle between requests.  

**Q: How does the KV cache shape production serving, and what two generation phases does it split into?**  `Advanced`  
The KV cache is the model's running memory of everything it has read, one entry per token. It dominates serving because it grows with context and eats GPU memory. Generation splits into two phases around it. Prefill reads the whole prompt in one pass to build the initial cache, and it's compute-bound. Decode then emits tokens one at a time, each step re-reading the growing cache, so it's memory-bandwidth-bound.  
...
There are two. Prefill is the model reading your entire prompt in a single parallel pass and storing a running summary of every token in the KV cache. Decode is the model then writing the answer one token at a time, with each step reusing that cache instead of re-reading the prompt.  
...
Model routing sends each incoming request to the cheapest model that can still answer it well. It makes the decision up front, before the real answer, using signals like predicted difficulty or task type. A cascade flips the order: it skips the guess, tries the small model first, checks the result, and escalates to a bigger model only when the result fails a confidence check.  

**Q: What defines a small language model (SLM), and what four benefits come from running one on-device?**  `Advanced`  
An SLM is a language model small enough to run directly on a phone or laptop, roughly 1-8 billion parameters, with no data centre involved. Running on-device gives four wins at once. Privacy, because your data never leaves the hardware. Low latency, because there's no network round-trip, so replies feel instant. Offline operation, so it works on a plane or in a tunnel. And near-zero marginal cost, because there's no per-token API bill.  

**Q: What does prompt caching reuse across requests, and how does that cut latency and cost?**  `Applied`  
It reuses the model's processed state for the fixed prefix of your prompt, meaning the system prompt, the rules, the examples, any long document that stays the same call after call. The first call processes it and stores that work. Later calls that start with the same prefix reuse the stored state instead of re-computing it. On cost, you are charged a small cache-read rate on those tokens instead of full input price. On latency, you skip most of the prefix's processing, so the first word comes back sooner.  

**Q: Design a customer-support RAG bot over a million documents. What's the top-level architecture?**  `Advanced`  
I'd split it in two. The offline ingestion half is a batch job: load the docs, chunk them, run each chunk through an embedding model, and store the vectors in a sharded vector database with an ANN index. That part is slow and expensive but only runs when documents change. The online serving half is the live request: embed the query, retrieve the closest chunks, rerank to the best few, build a prompt, generate, and cite, with a semantic cache in front.  
...
There are four everyday moves. Caching cuts cost: prompt caching reuses the model's work on a repeated prefix, and a semantic cache returns a saved answer for a repeat question without any model call. Streaming makes the app feel fast, since the first token appears almost instantly while total time is unchanged. Model routing sends easy questions to a cheap small model and escalates only the hard ones. And fallbacks with timeouts and backed-off retries keep the app up when a provider degrades.  

**Q: What is LLM observability, and why isn't ordinary monitoring enough?**  `Advanced`  
Traditional monitoring watches error rates, response times, and CPU on simple request/response servers. LLM apps break that model. One question fans out into a long chain of model calls, retrievals, and tool uses, and an agent may loop and retry. Observability captures the whole session as a structured trace, so failures buried inside multi-step reasoning become visible and replayable.  
...
Cost is driven by tokens. The bill is roughly input tokens plus output tokens, each multiplied by its per-token price. Providers split the price because the two halves cost them differently. Reading your input is cheap, but generating each new output token is the expensive compute, so output is quoted 2 to 4× higher.  

**Q: What is LLM FinOps, and what three moves does it combine?**  `Advanced`  
LLM FinOps is the discipline of treating token spend like a managed budget so it never surprises you. It combines three moves. You attribute, tagging every model call by feature, team, and customer so the bill becomes a breakdown. You cut, deliberately shrinking cost with caching, cheaper models, and leaner prompts, measured before and after. And you cap, enforcing hard daily spend limits per feature with a circuit breaker.  

**Q: Before any AI is involved, what makes an LLM app 'just a normal web service', and what does /v1 buy you?**  `Advanced`  
Strip out the model and it is a standard REST service. The untrusted browser calls your endpoint, which holds the key, checks auth, meters usage, and logs. The /v1 is API versioning. It lets you ship a v2 with a new response shape on a new path while old clients keep calling /v1 unbroken.  
...
Almost all of it is ordinary web work. You containerize the FastAPI and RAG service, push the image to a host, inject config with env vars and secrets, run stateless replicas behind a load balancer, and keep state in a managed DB and object storage. The one twist is that model inference is heavy. It needs a GPU, costs real money, and scales differently from the light app layer, so you keep inference behind a hosted API or a separate GPU pool rather than glued to your web boxes.  

**Q: How does an automated eval gate in CI block a bad release, and what does it compare against?**  `Advanced`  
A GitHub Actions workflow triggers on push: build, unit and integration tests, then the eval gate. The gate scores the candidate on a fixed golden set, often with an LLM judge on a pinned seed for repeatability, and compares to the stored baseline, the last prod score. A drop beyond tolerance makes the job exit non-zero, which fails the required check and blocks the merge.  
...
Authentication answers who you are: you verify a signed JWT or a session cookie from login. Authorization answers what you're allowed to do, usually through RBAC roles like admin, member, and viewer. If you mis-identify the caller, every permission check that follows is deciding access for the wrong person, so the whole chain fails together.  
...
A call is a list of messages with three roles. system sets the durable rules and format, user is the request, and assistant is prior model turns you replay for context. The endpoint is stateless and keeps nothing between calls, so you resend the entire thread each time.  
...
Prompt injection is when text the model reads gets it to follow the attacker's orders instead of yours. The root cause is that a model reads its prompt as one flat sequence of tokens. Your trusted rules and the untrusted data sit side by side, and nothing in the math marks one as more authoritative.  
...
A trained model is over-provisioned. Lots of its weights are near zero and contribute almost nothing. Pruning finds those barely-used parts and removes them, sometimes whole neurons or heads, so the model gets smaller and faster while saying almost the same things. What you are left with is sparsity: a network where most weights are zero and can be skipped at compute time.  
